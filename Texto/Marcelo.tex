...
\titulo{Análise de geradores de números aleatórios e pseudoaleatórios através de técnicas da teoria da informação}
...
We analyze the dispersion of the characteristic points of these sequences in the plane ($H\times C$) using four factors: the size of the sequence ($N$), the size of the word ($D$), the delay ($\tau$) and the generating source, observing the distance from the characteristic points to the reference point.

Then evidence suggests that the source would be a factor that is irrelevant to the analysis. In order to consolidate this possibility, we applied the Kolmogorov-Smirnov test to pairs of comparable sequences, and we verify it.
