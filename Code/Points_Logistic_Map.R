...
Log_Map1k <- logisticmap(1000, .01)
# Log_Map50k <- logisticmap(50000, .01)
...
for(r in 3:6) {
  for(t in c(1,10,30,50)) {
    Points_Est1k <- rbind(Points_Est1k, c(r,t,complexity_entropy(Log_Map1k, r, t)))
  }  
}
# Stop the clock
proc.time() - ptm
...
# for(r in 3:6) {
#   for(t in c(1,10,30,50)) {
#     Points_Est50k <- rbind(Points_Est50k, c(r,t,complexity_entropy(Est50k, r, t)))
#   }  
# }
# # Stop the clock
# proc.time() - ptm
...
ggsave("ConfidInt_mapa_logistico_1k_t1.png", plot = last_plot(), device = "png", path = "../newPlots", scale = 1, dpi = 300, limitsize = TRUE)
