## To test the MT and Randu points against the test
...
attach(HC_no_MT)
quant = function(x){quantile(x, probs=c(1/1000, 10/1000, 50/1000, 100/1000, 900/1000, 950/1000, 990/1000, 999/1000), type=4)}
quantis.por.fator <- summarize(dEuclid, llist(Source, N, D, tau), quant)
names(quantis.por.fator) <- c( "Source", "N", "D", "tau", "0.1%", "1%", "5%", "10%", "90%", "95%", "99%", "99.9%")
#detach(HC_no_MT)
...
names(HCN1000tau1meltQuant) <- c("D", "0.1%", "1%", "5%", "10%", "90%", "95%", "99%", "99,9%") 
...
ggplot() +
  geom_line(data=subset(tt, D==3), aes(D, value, color=variable, group=variable), alpha=.3) +
  ...
  geom_segment(data=subset(tt, D==3), aes(as.numeric(D)-.05, value, xend = as.numeric(D)+.05, yend = value)) +
  geom_point(data=subset(Points_Randu_melt, D==3), aes(D,value), colour = "red", size = 2) +
...
ggsave("ConfidInt_MT_1k_t1.png", plot = last_plot(), device = "png", path = "../newPlots", scale = 1, dpi = 300, limitsize = TRUE)
...
ggsave("ConfidInt_Randu_1k_t1.png", plot = last_plot(), device = "png", path = "../newPlots", scale = 1, dpi = 300, limitsize = TRUE)
