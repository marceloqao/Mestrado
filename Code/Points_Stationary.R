...
for(r in 3:6) {
  for(t in c(1,10,30,50)) {
    Points_Est1k <- rbind(Points_Est1k, c(r,t,complexity_entropy(Est1k, r, t)))
  }  
}
# Stop the clock
proc.time() - ptm
...
ggsave("ConfidInt_estacionaria_1k_t1.png", plot = last_plot(), device = "png", path = "../newPlots", scale = 1, dpi = 300, limitsize = TRUE)
