...
load(file='Data/HC_Random_All.gzip')
load(file='Data/inf.gzip')
load(file='Data/sup.gzip')

# Determinação de limite
minH <- min(HC_All_Random$H)
maxC <- max(HC_All_Random$C)

# Acrescentando a distância euclidiana ao data.frame
attach(HC_All_Random)
HC_All_Random$D <- as.factor(D)
HC_All_Random$dEuclid = sqrt((H-1)^2 + JS^2)
detach(HC_All_Random)

# Visualização global, com transparência
ggplot(data=HC_All_Random, aes(x=H, y=C)) + 
...
D.current <- 3
tau.current <- 1
HC_subset <- subset(HC_All_Random, D==D.current & tau==tau.current)
...
data <- melt(HC_All_Random, measure.vars = "dEuclid")
