...
Fig.~\ref{fig:AllSystems} shows the Entropy-Complexity ($H\times C$) plane with bounds for $D=6$.
...
\item Deterministic series: monotonic increasing ($\log(x_t+0.1)$) and periodic ($\sin(2x_t)\cos(2x_t)$), with $0\leq x_t\leq 2\pi$ over ten thousand equally spaced points.
\end{itemize}
...
\begin{figure}[hbt]
\centering
\subfloat[Eleven systems and their points in the $H\times C$ plane\label{fig:AllSystems}]{\includegraphics[width=\linewidth]{AllSystems}}\\
\subfloat[White Noise, $f^{-1/2}$ and $f^{-1}$ noise points\label{fig:RightMostCorner}]{\includegraphics[width=\linewidth]{RightMostCorner}}
